...
if __name__ == "__main__":
    ...
    assert "vertices_wotrans" in result
    assert "keypoints3d" in result
    params_batch = {
        "rot6d": torch.randn(3, 100, 22, 6),
        "trans": torch.randn(3, 100, 3),
    }
    result_batch = model.forward_batch(params_batch)
    assert result_batch["vertices"].shape[0] == 3
    assert result_batch["vertices_wotrans"].shape[0] == 3
    assert result_batch["keypoints3d"].shape[0] == 3
