...
def row_6d_to_col_6d(row_d6: torch.Tensor) -> torch.Tensor:
    """
    Converts row-based 6D rotations to column-interleaved 6D rotations.
    """
    # Orthonormalize as rows to get rotation matrix R
    R = rotation_6d_to_matrix(row_d6)
    # Extract first two columns
    c1, c2 = R[..., :, 0], R[..., :, 1]
    cols = torch.stack((c1, c2), dim=-1)
    return cols.flatten(-2)

def col_6d_to_row_6d(col_d6: torch.Tensor) -> torch.Tensor:
    """
    Converts column-interleaved 6D rotations to row-based 6D rotations.
    """
    # Parse as two columns
    cols = col_d6.view(*col_d6.shape[:-1], 3, 2)
    c1, c2 = cols[..., 0], cols[..., 1]
    # Orthonormalize as columns
    b1 = F.normalize(c1, dim=-1)
    b2 = c2 - (b1 * c2).sum(dim=-1, keepdim=True) * b1
    b2 = F.normalize(b2, dim=-1)
    b3 = torch.cross(b1, b2, dim=-1)
    # Stack as columns to form R
